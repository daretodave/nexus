#!/usr/bin/env node
// scripts/new-skill.mjs — the kit's own skill scaffolder.
//
// concepts/skills-anatomy.md documents the canonical two-file
// skill shape (skills/<verb>.md + .claude/commands/<verb>.md), but
// every skill so far was hand-copied from a sibling and
// hand-renamed — exactly how anatomy drift starts. This emits both
// files pre-filled with the canonical sections instead, TODO-marked
// throughout: it scaffolds structure, not procedure. The author
// still writes the actual contract.
//
...
// skills/ + .claude/commands/ — for scaffolding a skill that ships
// to adopters rather than one this repo runs on itself. Refuses to
// overwrite either target file if it already exists — delete both
// first if you really mean to regenerate.
